...
{ int fast=1;
  ...
  // Call newProcess to compile the process and store the address
  processAddress = newProcess("process_23_and_24");

  // Check if the process was successfully compiled
  if (processAddress == NULL) {
      printf("Error: newProcess could not be compiled.\n");
      return 1;
  }

  vSigmaCC(T, processAddress, 0)
  
  
  double Xf;
  Omega=darkOmegaExt(&Xf,fast,Beps,&err);
